...
function makePosts(postArr, page) {
  ...
  let postIndexArr = []; //요청한 페이지에 해당하는 인덱스
  // 페이지 파라미터가 빈값으로 들어왔다면 1로 맞추기
  if(page == "") {
    page = 1;
  }
  // 페이지에 해당되는 게시글 인덱스 구하기
  for(let i = 1; i < 11; i++) {
    postIndexArr.push(page*10-i);
  }
  // 페이지 인덱스에 맞는 게시글 만들기
  for(let i = 0; i < 10; i++) {
    if(postArr[postIndexArr[i]] == null) { continue; }
    let str = '<tr class="post" onclick=location.href="/view/support/post/post.html?post_index=' + postIndexArr[i] + '">'
    + '<th class="post_number">' + (postIndexArr[i] + 1) + '</th>'
    + '<td class="post_title">' + postArr[postIndexArr[i]].title + '</td>'
